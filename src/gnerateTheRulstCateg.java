...
public class gnerateTheRulstCateg {
	...
	public static void main(String[] args) throws IOException {
		...
		double prdicedrate1 = 0;
		double prdicedrate2=0;
		int Catog = 0;
		int rate = 0;
		float usmofrate = 0;
		...
		for (int i = 0; i < streamRanting.length; i++) {
			...
			for (int ii = 0; ii < 29; ii++) {
				prdicedrate += sumofCteg[ii] * algorhtmArryAll[ii];
				 prdicedrate2 += sumofCteg[ii] * algorhtmArryPart[ii];
			}
			// add the baise
			...
			prdicedrate1 = neuralClass.getNeural(nerual);

			prdicedrate += algorhtmArryAll[29];
			prdicedrate2 += algorhtmArryPart[29];
			...
			finalresult[i][4] = (float) prdicedrate1;
			finalresult[i][5] = (float) prdicedrate2;
			...
			// regression basline
				...
				sum_y+=y;
				sum_xy+= x*y;
				b= (((x*sum_xy)- sum_x*sum_y))/((x*sum_x_2)-(sum_x*sum_x));
				a=(sum_y-b*sum_x)/x;
			...
			System.out.printf("%.0f \t", finalresult[i][0]);
			System.out.printf("%.2f \t", finalresult[i][1]);
			System.out.printf("%.2f \t", finalresult[i][2]);
			System.out.printf("%.2f \t", finalresult[i][3]);
			System.out.printf("%.2f \t", finalresult[i][4]);
			System.out.printf("%.2f \t", finalresult[i][5]);
			System.out.printf("%.2f \t", finalresult[i][6]);
