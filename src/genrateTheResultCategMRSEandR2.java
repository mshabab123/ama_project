...
public class genrateTheResultCategMRSEandR2 {

	genrateTheResultCategMRSEandR2(String streamRanting[][]) {
		...
		int Catog = 0;
		int rate = 0;
		float usmofrate = 0,usmofrate1=0;
		...
		/*******************************
		to change the code form usual rsme to predict sideom of crowd 
		you have to cange the sidomofcrowd instead of finalresult[i][2]
		/**********************************/
		...
		for (int i = 0; i < streamRanting.length; i++){
		 ...
		 float widomOfcrowd = usmofrate1 / (streamRanting.length);

		/*******************************/
		 int instance = streamRanting.length;
		...
		rmse_svm = (float) Math.sqrt(rmse_svm / instance);
		rsme_nearl = (float) Math.sqrt(rsme_nearl / instance);
		rsme_part = (float) Math.sqrt(rsme_part / instance);
		rsem_basline = (float) Math.sqrt(rsem_basline / instance);

		System.out.println( rmse_svm+"\t"+rsme_nearl + "\t"
				+ rsme_part + "\t " + rsem_basline);

		
		try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("final_all", true)))) {
		    out.println(rmse_svm+"\t"+rsme_nearl + "\t"
					+ rsme_part + "\t " + rsem_basline+"\t"+instance);
		}catch (IOException e) {
